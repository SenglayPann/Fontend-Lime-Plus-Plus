...
export default async function ProjectDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!session?.user?.accessToken) {
    return (
      <DashboardLayout>
        <p className="text-muted-foreground">You need to sign in to view this project.</p>
      </DashboardLayout>
    );
  }
  ...
  const prAcceptance = pullRequests.length > 0 ? Math.round((mergedPrs / pullRequests.length) * 100) : 0;
  const members = project.members || [];
  const memberCount = members.length || project._count?.members || 0;
  const averageTaskCompletion = memberCount > 0 ? Math.round((completedTasks / Math.max(memberCount, 1)) * 10) : 0;
  ...
  for (const member of members) {
    const name = member.user?.name || member.user?.githubUsername || member.user?.email || "Unnamed";
    tasksByAssignee.set(name, 0);
  }
  for (const task of tasks) {
    const name = task.assignee?.name || task.assignee?.githubUsername || task.assignee?.email || "Unassigned";
    tasksByAssignee.set(name, (tasksByAssignee.get(name) || 0) + (task.status === "DONE" ? 1 : 0));
  }
  ...
  const chartData = contributionData.length > 0 ? contributionData : [{ name: "No Activity", value: 1 }];
  const leaderboardData = teamData.length > 0 ? teamData : [{ name: "No Members", score: 0 }];
      ...
      <div className="space-y-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-muted-foreground">
              {project.department?.name || "No department"}{project.repository ? ` · ${project.repository}` : ""}
            </p>
          </div>
          ...
          <div className="flex flex-wrap items-start gap-3">
            ...
            <ProjectActions projectId={id} accessToken={token} isLocked={project.status === "LOCKED"} />
          </div>
        </div>

        {(tasksResult.error || prsResult.error) && (
          <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
            Some related project data could not be loaded: {tasksResult.error || prsResult.error}
          </div>
        )}
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            { label: "Active Members", value: memberCount, icon: Users, color: "text-blue-600" },
            { label: "PR Acceptance", value: `${prAcceptance}%`, icon: Github, color: "text-purple-600" },
            { label: "Avg. Done Tasks", value: averageTaskCompletion / 10, icon: Trophy, color: "text-amber-500" },
          ].map((stat) => (
            <Card key={stat.label}>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Current project activity from tasks and pull requests.</CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Completed tasks by project member.</CardDescription>
            </CardHeader>
            <CardContent>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <Card className="lg:col-span-2">
            <CardHeader>
              ...
              <CardDescription>Live task status distribution and evaluation window.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              <div className="grid gap-3 sm:grid-cols-4">
                {[
                  ...
                  ["Open", Math.max(totalTasks - completedTasks - activeTasks - blockedTasks, 0)],
                ].map(([label, value]) => (
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="rounded-lg bg-muted/30 p-4">
                  <p className="text-xs text-muted-foreground">Evaluation Start</p>
                  <p className="font-medium">
                    {project.evalStart ? new Date(project.evalStart).toLocaleDateString() : "Not set"}
                  </p>
                </div>
                <div className="rounded-lg bg-muted/30 p-4">
                  <p className="text-xs text-muted-foreground">Evaluation End</p>
                  <p className="font-medium">
                    {project.evalEnd ? new Date(project.evalEnd).toLocaleDateString() : "Not set"}
                  </p>
                </div>
              </div>
            </CardContent>
            ...
            <CardContent className="space-y-6">
              <div className="flex items-center gap-4 rounded-lg border border-border p-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate">{project.repository || "No repository"}</p>
                  <p className="text-xs text-muted-foreground">
                    {project.platform || "GITHUB"}{project.externalProjectId ? ` · ${project.externalProjectId}` : ""}
                  </p>
                </div>
                <Button variant="ghost" size="icon" disabled={!repoUrl} asChild={!!repoUrl}>
                  {repoUrl ? (
                    <a href={repoUrl} target="_blank" rel="noreferrer" aria-label="Open repository">
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">{pullRequests.length || project._count?.pullRequests || 0}</span>
                </div>
              </div>

              <Button className="w-full gap-2" variant="outline" disabled={!repoUrl} asChild={!!repoUrl}>
                {repoUrl ? (
