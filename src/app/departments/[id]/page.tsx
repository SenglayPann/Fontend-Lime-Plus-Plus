...
async function fetchDepartmentDashboard(id: string, token: string) {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/dashboard/departments/${id}`,
      {
    ...
    return (
      <DashboardLayout>
        <p className="text-muted-foreground">
          You need to sign in to view this department.
        </p>
      </DashboardLayout>
    );
  }

  const result = await fetchDepartmentDashboard(id, session.user.accessToken);

  return (
    <DashboardLayout>
      {result.error || !result.data ? (
        <div className="rounded-lg border border-destructive/20 bg-destructive/10 p-4 text-sm font-medium text-destructive">
          {result.error || "Department dashboard not found"}
        </div>
      ) : (
        <ScopeDashboardView data={result.data} backHref="/departments" />
      )}
    </DashboardLayout>
  );
}
